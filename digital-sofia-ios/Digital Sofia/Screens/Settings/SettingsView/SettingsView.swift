    ...
    var body: some View {
        LoadingStack(isPresented: $viewModel.isLoading) {
            VStack(spacing: 0) {
                ...
                NavigationLink(destination: ProfileDeleteView(canDeleteUser: $viewModel.canDeleteUser).environmentObject(appState).environmentObject(networkMonitor),
                               isActive: $viewModel.navigateToDeleteUser) { EmptyView() }
