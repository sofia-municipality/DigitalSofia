...
const Edit = React.memo(() => {
  ...
  const formHistory = useSelector(
    (state) => state.formRestore?.formHistory || []
  );
  ...
  const saveNewVersion = (
    <Translation>{(t) => t("Save New Version")}</Translation>
  );
  ...
  useEffect(() => {
    if (processListData?.parentFormId && !formHistory.length) {
      getFormHistory(processListData?.parentFormId)
        .then((res) => {
  ...
  const isMapperSaveNeeded = (newData) => {
    ...
    return (
      ...
      prviousData.formPath !== newData.path ||
        ...
        dispatch(
          saveFormProcessMapperPost(data, (err) => {
            if (!err) {
              ...
              dispatch(
                push(`${redirectUrl}formflow/${submittedData._id}/preview`)
              );
            }
          })
        );
  ...
  const saveFormData = () => {
    ...
    const newFormData = manipulatingFormData(
      ...
      form.submissionAccess?.length ? form.submissionAccess : submissionAccess,
      !form.submissionAccess?.length ? submissionAccessResource : null
    );
