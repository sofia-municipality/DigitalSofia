    ...
    <>
      <Modal show={modalOpen} onHide={onClose}>
        ...
        <Modal.Body>
          ...
          >
            {`${formUploadCounter} of ${formUploadList.length} ${
            ...
            formUploadCounter + formUploadFailureCounter ? (
              <Spinner animation="border" variant="primary" />
            ) : (
              ""
            )}
            {formUploadFailureCounter !== 0 && (
              <span className="fileupload-fail">
                {formUploadFailureCounter !== 0
                  ? `Failed to upload ${formUploadFailureCounter} ${
                      formUploadFailureCounter > 1 ? "forms...!" : "form...!"
                    }`
                  : ""}
              </span>
            )}
          ...
          {formUploadList.length ? (
            ...
            >
              <div
                ...
                style={{
                  width: `${
                    formsUploaded === 100 ? "100%" : `${formsUploaded}%`
                  }`,
                  backgroundColor: `${formsUploaded === 100 ? "#4d53fa" : ""}`,
                }}
              ></div>
            </div>
          ) : (
        ...
        <Modal.Footer
          style={{ justifyContent: `${noAccess ? "space-between" : ""}` }}
        >
          {noAccess && (
            <span className="fileupload-fail">
              {t("Access restricted by its designer..!")}
            </span>
          )}
          <Button
            style={{ width: "20%" }}
