...
export const updateApplicationEvent = (url, data, ...rest) => {
  /* * Data Format
 {
...
export const sendProcessEvent = async ({ messageName, processInstanceId }) => {
  const res = await httpPOSTRequest(API.BPM_SENT_MESSAGE, {
    messageName,
    processInstanceId,
  });
  return res.data;
...
export const checkApplicationPermissions = async ({
  formioFormId,
  taskId,
  callback,
}) => {
  try {
    const res = await httpGETRequest(API.APPLICATION_CHECK_PERMISSIONS, {
      formioFormId,
      taskId,
    });
...
export const updateApplicationStatus = async ({
  ...
  const url = API.UPDATE_APPLICATION_STATUS.replace(
    "<application_id>",
    applicationId
  );
  try {
    const res = await httpPUTRequest(url, {
      applicationStatus,
      formUrl,
    });
    callback(null, res.data);
...
export const deleteApplicationById = (applicationId) => {
  let url = API.DELETE_APPLICATION.replace("<application_id>", applicationId);
  return httpDELETERequest(url);
};

export const withdrawApplication = ({ applicationId, role }) => {
  let url = API.APPLICATION_WITHDRAW.replace(
    "<application_id>",
    applicationId
  ).replace("<role>", role);
