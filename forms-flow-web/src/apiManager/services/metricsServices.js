      ...
      .then((res) => {
        if (res.data) {
          ...
          if (res.data.applications && res.data.applications[0]) {
            dispatch(
              setSelectedMetricsId(res.data.applications[0].parentFormId)
            );
          } else {
...
export const fetchMetricsSubmissionStatusCount = (
  ...
  return (dispatch) => {
    ...
    httpGETRequest(
      `${
        API.METRICS_SUBMISSIONS
      }/${id}?from=${fromDate}&to=${toDate}&orderBy=${setSearchBy}&formType=${
        options.parentId ? "parent" : "form"
      }`
    )
      .then((res) => {
        if (res.data) {
