...
export const getTaskSubmitFormReq = (
  formUrl,
  applicationId,
  actionType,
  webFormUrl
...
export const checkIfProcessExists = async (processKey, tenantId) => {
  const url = `${API.GET_PROCESS_DEFINITION.replace(
    "<process_key>",
    processKey
  )}/xml`;
  const data = {};
  ...
  try {
    const res = await httpGETRequest(url, data);
    return !!res.data;
  } catch (err) {
    return false;
  }
