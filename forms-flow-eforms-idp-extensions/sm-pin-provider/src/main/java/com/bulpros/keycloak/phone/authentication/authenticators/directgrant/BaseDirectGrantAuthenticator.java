...
public abstract class BaseDirectGrantAuthenticator implements Authenticator {
    ...
    protected String getGrantType(AuthenticationFlowContext context) {
        return context.getHttpRequest().getDecodedFormParameters().getFirst("grant_type");
    }

    protected String getClientId(AuthenticationFlowContext context) {
        return context.getHttpRequest().getDecodedFormParameters().getFirst("client_id");
    }

    protected String getScope(AuthenticationFlowContext context) {
        return context.getHttpRequest().getDecodedFormParameters().getFirst("scope");
    }
    ...
    protected void invalidCredentials(AuthenticationFlowContext context, Response.Status status, String errorMessage,
