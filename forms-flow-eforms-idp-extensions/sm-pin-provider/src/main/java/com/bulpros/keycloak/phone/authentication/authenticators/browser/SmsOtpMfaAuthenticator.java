...
import static com.bulpros.keycloak.phone.authentication.authenticators.browser.PhoneUsernamePasswordForm.VERIFIED_PHONE_NUMBER;
...
public class SmsOtpMfaAuthenticator implements Authenticator, CredentialValidator<PhoneOtpCredentialProvider> {
    ...
    private static final String PAGE = "loading.ftl";
    ...
    protected boolean validateCookie(AuthenticationFlowContext context) {
        ...
        var invalid = PhoneOtpCredentialModel.getSmsOtpCredentialData(context.getUser())
                .map(PhoneOtpCredentialModel.SmsOtpCredentialData::isSecretInvalid).orElse(true);

        if (invalid)
            return false;

        return Optional.of(context.getHttpRequest().getHttpHeaders().getCookies()).flatMap(
                cookies -> Optional.ofNullable(cookies.get("SMS_OTP_ANSWERED"))
                        .flatMap(cookie -> OptionalUtils.ofBlank(cookie.getValue())).flatMap(
                                credentialId -> Optional.ofNullable(cookies.get(credentialId))
                                        .flatMap(cookie -> OptionalUtils.ofBlank(cookie.getValue()))
                                        .map(secret -> context.getUser().credentialManager().isValid(
                                                new UserCredentialModel(credentialId, getType(context.getSession()),
                                                        secret))))).orElse(false);
    }
    ...
    protected void setCookie(AuthenticationFlowContext context, String credentialId, String secret) {
        ...
        addCookie(context, "SMS_OTP_ANSWERED", credentialId, uri.getRawPath(), null, null, maxCookieAge, false, true);
        addCookie(context, credentialId, secret, uri.getRawPath(), null, null, maxCookieAge, false, true);
    }
    ...
    public void authenticate(AuthenticationFlowContext context) {
        ...
        boolean verified = OptionalUtils.ofBlank(context.getAuthenticationSession().getAuthNote(VERIFIED_PHONE_NUMBER))
                .map(number -> number.equalsIgnoreCase(phoneNumber)).orElse(false);
        if (verified) {
            context.success();
            return;
        }

        //    PinProvider phoneProvider = context.getSession().getProvider(PinProvider.class);
        //    try {
    ...
    public void action(AuthenticationFlowContext context) {
        ...
        if (credentialId == null || credentialId.isEmpty()) {
            var defaultOtpCredential = getCredentialProvider(context.getSession()).getDefaultCredential(
        ...
        if (Validation.isBlank(secret)) {
            context.form().setError(SupportPhonePages.Errors.NOT_MATCH.message());
            Response challenge = challenge(context, phoneNumber);
    ...
    protected Response challenge(AuthenticationFlowContext context, String phoneNumber) {
        return context.form().setAttribute(SupportPhonePages.ATTRIBUTE_SUPPORT_PHONE, true)
                .setAttribute(SupportPhonePages.ATTEMPTED_PHONE_NUMBER, phoneNumber).createForm(PAGE);
    }
