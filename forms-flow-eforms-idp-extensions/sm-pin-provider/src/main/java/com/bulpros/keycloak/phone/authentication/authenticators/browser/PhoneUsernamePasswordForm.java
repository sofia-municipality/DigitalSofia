...
public class PhoneUsernamePasswordForm extends UsernamePasswordForm implements Authenticator, AuthenticatorFactory {
    ...
    private boolean isLoginWithPhoneNumber(AuthenticationFlowContext context) {
        return context.getAuthenticatorConfig() == null || context.getAuthenticatorConfig().getConfig()
                .getOrDefault(CONFIG_IS_LOGIN_WITH_PHONE_NUMBER, "true").equals("true");
    }
    ...
    private boolean isSupportPhone(AuthenticationFlowContext context) {
        return context.getAuthenticatorConfig() == null || context.getAuthenticatorConfig().getConfig()
                .getOrDefault(CONFIG_IS_LOGIN_WITH_PHONE_VERIFY, "true").equals("true");
    }
    ...
    protected boolean validateForm(AuthenticationFlowContext context, MultivaluedMap<String, String> inputData) {
        ...
        if (Validation.isBlank(phoneNumber)) {
            ...
            Response challengeResponse = challenge(context, SupportPhonePages.Errors.MISSING.message(),
    ...
    private void invalidVerificationCode(AuthenticationFlowContext context, String number) {
        ...
        Response challengeResponse = challenge(context, SupportPhonePages.Errors.NOT_MATCH.message(),
                SupportPhonePages.FIELD_VERIFICATION_CODE);
    ...
    private boolean validatePhone(AuthenticationFlowContext context, String phoneNumber, String code) {
        ...
        try {
            ...
            return Utils.findUserByPhone(context.getSession(), context.getRealm(), validPhoneNumber)
                    .map(user -> validateVerificationCode(context, user, validPhoneNumber, code) && validateUser(
                            context, user, validPhoneNumber)).orElseGet(() -> {
                        context.getEvent().error(Errors.USER_NOT_FOUND);
                        context.form().setAttribute(SupportPhonePages.ATTEMPTED_PHONE_ACTIVATED, true)
                                .setAttribute(SupportPhonePages.ATTEMPTED_PHONE_NUMBER, phoneNumber);
                        assemblyForm(context, context.form());
                        Response challengeResponse = challenge(context,
                                SupportPhonePages.Errors.USER_NOT_FOUND.message(),
                                SupportPhonePages.FIELD_PHONE_NUMBER);
            ...
            context.form().setAttribute(SupportPhonePages.ATTEMPTED_PHONE_ACTIVATED, true)
                    .setAttribute(SupportPhonePages.ATTEMPTED_PHONE_NUMBER, phoneNumber);
            assemblyForm(context, context.form());
            Response challengeResponse = challenge(context, e.getErrorType().message(),
                    SupportPhonePages.FIELD_PHONE_NUMBER);
            context.failureChallenge(AuthenticationFlowError.INVALID_USER, challengeResponse);
            return false;
        }
    ...
    public boolean validateUserAndPassword(AuthenticationFlowContext context,
        ...
        return user != null && validatePassword(context, user, inputData,
                shouldClearUserFromCtxAfterBadPassword) && validateUser(context, user, inputData);
    }
    ...
    private UserModel getUserFromForm(AuthenticationFlowContext context, MultivaluedMap<String, String> inputData) {
        ...
        try {
            ...
            if (user == null && isLoginWithPhoneNumber(context) && !Utils.isDuplicatePhoneAllowed(
                    context.getSession())) {
    ...
    static {
        CONFIG_PROPERTIES = ProviderConfigurationBuilder.create().property().name(CONFIG_IS_LOGIN_WITH_PHONE_VERIFY)
