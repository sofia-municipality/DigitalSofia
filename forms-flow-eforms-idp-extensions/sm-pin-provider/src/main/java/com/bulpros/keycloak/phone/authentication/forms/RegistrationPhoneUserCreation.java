package com.bulpros.keycloak.phone.authentication.forms;
...
import static com.bulpros.keycloak.phone.authentication.forms.SupportPhonePages.FIELD_PHONE_NUMBER;
...
public class RegistrationPhoneUserCreation implements FormActionFactory, FormAction {
    ...
    static {
        CONFIG_PROPERTIES = ProviderConfigurationBuilder.create().property().name(CONFIG_PHONE_NUMBER_AS_USERNAME)
                .type(BOOLEAN_TYPE).label("Phone number as username").helpText(
                        "Allow users to set phone number as username. If Realm has `email as username` set to true, this is invalid!")
                .defaultValue(true).add().property().name(CONFIG_INPUT_NAME).type(BOOLEAN_TYPE).label("Input name")
                .helpText("Allow users to input first and last name.").defaultValue(true).add().property()
                .name(CONFIG_INPUT_EMAIL).type(BOOLEAN_TYPE).label("Input Email")
                .helpText("Allow users to input e-mail. If Realm has `email as username` set to true, this is invalid!")
                .defaultValue(true).add().build();
    }
    ...
    private boolean isPhoneNumberAsUsername(FormContext context) {
        if (context.getAuthenticatorConfig() == null || "true".equals(
                context.getAuthenticatorConfig().getConfig().getOrDefault(CONFIG_PHONE_NUMBER_AS_USERNAME, "true"))) {
    ...
    private boolean isHideName(FormContext context) {
        return context.getAuthenticatorConfig() == null || !"true".equalsIgnoreCase(
                context.getAuthenticatorConfig().getConfig().getOrDefault(CONFIG_INPUT_NAME, "true"));
    }

    private boolean isHideEmail(FormContext context) {
        if (context.getAuthenticatorConfig() == null || "true".equalsIgnoreCase(
                context.getAuthenticatorConfig().getConfig().getOrDefault(CONFIG_INPUT_EMAIL, "true"))) {
            return false;
        }
        if (context.getRealm().isRegistrationEmailAsUsername()) {
    ...
    public void validate(ValidationContext context) {
        ...
        if (Validation.isBlank(phoneNumber)) {
            ...
            try {
                ...
                if (!Utils.isDuplicatePhoneAllowed(session) && Utils.findUserByPhone(session, context.getRealm(),
    ...
    public void success(FormContext context) {
        ...
        context.getEvent().detail(Details.USERNAME, username).detail(Details.REGISTER_METHOD, "form")
