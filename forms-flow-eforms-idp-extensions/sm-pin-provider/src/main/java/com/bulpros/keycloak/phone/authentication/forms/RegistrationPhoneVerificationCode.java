...
import static com.bulpros.keycloak.phone.authentication.forms.SupportPhonePages.FIELD_VERIFICATION_CODE;
...
public class RegistrationPhoneVerificationCode implements FormAction, FormActionFactory {
    ...
    static {
        CONFIG_PROPERTIES = ProviderConfigurationBuilder.create().property().name(CONFIG_OPT_CREDENTIAL)
                .type(BOOLEAN_TYPE).defaultValue(false).label("Create OTP Credential")
                .helpText("Create OTP credential by phone number.").add().build();
    }
    ...
    private final static Requirement[] REQUIREMENT_CHOICES = { Requirement.REQUIRED, Requirement.DISABLED };
    ...
    private PinVerificationCodeProvider getTokenCodeService(KeycloakSession session) {
    ...
    public void validate(ValidationContext context) {
        ...
        if (Validation.isBlank(verificationCode) || tokenCode == null || !tokenCode.getCode()
    ...
    public void success(FormContext context) {
        ...
        if (config != null && "true".equalsIgnoreCase(
                config.getConfig().getOrDefault(CONFIG_OPT_CREDENTIAL, "false"))) {
