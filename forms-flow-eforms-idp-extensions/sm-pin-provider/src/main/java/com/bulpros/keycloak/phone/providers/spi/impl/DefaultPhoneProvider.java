...
public class DefaultPhoneProvider implements PinProvider {
    ...
    @Override
    public int sendTokenCode(String pin,String sourceAddr,TokenCodeType type, String kind){

        logger.info("send code to pnone with pin:" + pin );
        ...
        TokenCodeRepresentation ongoing = getTokenCodeService().ongoingProcess(pin, type);
        if (ongoing != null) {
            logger.info(String.format("No need of sending a new %s code for pin %s",type.label, pin));
            return (int) (ongoing.getExpiresAt().getTime() - Instant.now().toEpochMilli()) / 1000;
        }

        TokenCodeRepresentation token = TokenCodeRepresentation.forPin(pin);

        try {
            session.getProvider(MessageSenderService.class, service).sendSmsMessage(type,pin,token.getCode(),tokenExpiresIn,kind);
            getTokenCodeService().persistCode(token, type, tokenExpiresIn);

            logger.info(String.format("Sent %s code to %s over %s",type.label, pin, service));

        } catch (MessageSendException e) {

            logger.error(String.format("Message sending to %s failed with %s: %s",
                    pin, e.getErrorCode(), e.getErrorMessage()));
