...
import { BASE_ROUTE, MULTITENANCY_ENABLED, TASK_PAGE_NEW_DESIGN_ENABLED } from "../../constants/constants";
...
const ServiceFlow = React.memo(() => {
  ...
  const taskListColWidths = TASK_PAGE_NEW_DESIGN_ENABLED ? {
    sm: 12
  } : {
    lg: 3,
    xs: 12, 
...
const ServiceFlowWrapper = ({props}) => {
  const currentUser = useSelector(
    (state) => state.user?.userDetail
  );

  return currentUser ? <ServiceFlow {...props} /> : <Loading />;
};

export default ServiceFlowWrapper;
