...
import { checkIfProcessExists } from "../../apiManager/services/bpmServices";
...
const List = React.memo((props) => {
  ...
  const submissionAccessResource = useSelector(
    (state) => state.user?.submissionAccessResource || []
  );
  ...
  const downloadForms = async () => {
    ...
    for (const form of formCheckList) {
      ...
      const proccessMapperData = await getFormProcessMapperData(form._id);
      newFormData = getFormattedForm(newFormData.data);
      newFormData.proccessMapperData = proccessMapperData;
      if (MULTITENANCY_ENABLED && tenantKey) {
        newFormData.path = newFormData.path.replace(`${tenantKey}-`, "");
        newFormData.name = newFormData.name.replace(`${tenantKey}-`, "");
      }
  ...
  const mapperHandler = async (form, proccessMapperData) => {
    const data = {
      formId: form._id,
      formName: form.title,
      formPath: form.path,
    ...
    if (proccessMapperData) {
      try {
        const processExists = await checkIfProcessExists(
          proccessMapperData.processKey,
          tenantKey
        );
        if (processExists) {
          data.anonymous = proccessMapperData?.anonymous || false;
          data.processKey = proccessMapperData?.processKey;
          data.processName = proccessMapperData?.processName;
          data.status = proccessMapperData?.status || INACTIVE;
          data.taskVariable = proccessMapperData?.taskVariable || [];
        }
      } catch (err) {
        console.log(err);
      }
  ...
  const isMapperSaveNeeded = (mapperData, formdata, applicationData, newMapperData) => {
    ...
    if (
      ...
      mapperData.formPath !== formdata.path ||
    ...
    if (
      newMapperData &&
      (newMapperData.status !== mapperData.status ||
        !_isEquial(newMapperData.taskVariable, mapperData.taskVariable) ||
        newMapperData.processKey !== mapperData.processKey ||
        newMapperData.processName !== mapperData.processName ||
        newMapperData.anonymous !== mapperData.anonymous)
    ) {
      return "update";
    }
  };
  ...
  const getFormSubmissionAccess = (anonymous) => {
    const mappedFormAccess = formAccess?.slice(0) || [];
    const mappedSubmissionAccess = submissionAccess?.slice(0) || [];
    const mappedSubmissionAccessResource =
      submissionAccessResource?.slice(0) || [];

    mappedFormAccess.forEach((role) => {
      if (anonymous) {
    ...
    mappedSubmissionAccess.forEach((access) => {
      if (anonymous) {
    ...
    mappedSubmissionAccessResource.forEach((access) => {
      if (anonymous) {
        if (access.type === "create_own") {
          access.roles.push(roleIds.ANONYMOUS);
        }
    ...
    return {
      mappedFormAccess,
      mappedSubmissionAccess,
      mappedSubmissionAccessResource,
    };
  };
        ...
        await Promise.all(
          fileContent.forms.map(async (formData) => {
            return new Promise((resolve, reject) => {
              let proccessMapperData;
              if (formData.proccessMapperData) {
                proccessMapperData = { ...formData.proccessMapperData };
                delete formData.proccessMapperData;
              }
              ...
              const {
                ...
                mappedSubmissionAccess,
                mappedSubmissionAccessResource,
              } = getFormSubmissionAccess(proccessMapperData?.anonymous);

              newFormData.access = mappedFormAccess;
              newFormData.submissionAccess =
                newFormData?.type === "resource"
                  ? mappedSubmissionAccessResource
                  : mappedSubmissionAccess;
              ...
              formCreate(newFormData)
                .then((res) => {
                  const { data } = res;
                  mapperHandler(data, proccessMapperData);
                  // call the auth api
                            ...
                            dispatch(
                              ...
                              getFormProcesses(formObj._id, (err, mapperData) => {
                                ...
                                if (mapperData) {
                                            ...
                                            .then((formupdated) => {
                                              ...
                                              const data = {
                                                ...
                                                processKey:
                                                  proccessMapperData?.processKey,
                                                processName:
                                                  proccessMapperData?.processName,
                                                parentFormId: mapperData.parentFormId,
                                                status:
                                                  proccessMapperData?.status
                                                    ? proccessMapperData?.status
                                                    : mapperData.status
                                                    ? mapperData.status
                                                    : INACTIVE,
                                                  taskVariable:
                                                    proccessMapperData?.taskVariable
                                                      ? proccessMapperData?.taskVariable
                                  ...
                                  formUpdate(formObj._id, newFormData)
                                    .then((res) => {
                                      if (res.data) {
