      ...
      .then(async (res) => {
        if (res.data) {
          ...
          if (!_embedded || !_embedded["task"] || !responseData["count"]) {
            ...
            let enrichedTaskData = taskData;
            if(TASK_PAGE_NEW_DESIGN_ENABLED) {
              enrichedTaskData = await Promise.all(taskData.map(async(task) => {
                try {
                  const vars = await getBPMTaskVariables(task.id);
                  const submissionUrl = vars?.taskFormUrl?.value || vars?.formUrl?.value;
                  if(!submissionUrl) {
            ...
            dispatch(setBPMTaskList(enrichedTaskData));
            dispatch(setBPMTaskLoader(false));
            done(null, enrichedTaskData);
          }
        } else {
          // console.log("Error", res);
          dispatch(setBPMTaskList([]));
...
const getBPMTaskVariables = async (taskId) => {
  ...
  const response = await RequestService.httpGETRequest(apiUrlgetTaskVariables);
