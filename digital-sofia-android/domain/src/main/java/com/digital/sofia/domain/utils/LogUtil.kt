...
import com.digital.sofia.domain.BuildConfig
...
object LogUtil {
    ...
    fun logError(message: String, error: ResultEmittedData.Error, tag: String) {
        CoroutineScope(Dispatchers.IO).launch {
            if (BuildConfig.DEBUG) {
                Log.e(
                    tag,
                    "$message Server code: ${error.responseCode} Server message: ${error.serverMessage}"
                )
                enqueue(
                    LogData.ErrorMessageWithData(
                        tag = tag,
                        time = System.currentTimeMillis(),
                        message = message,
                        error = error,
                    ...
                    queue.forEach {
                        when (it) {
                            ...
                            is LogData.ErrorMessageWithData -> {
                                ...
                                append("Server code:")
                                append(NEW_LINE)
                                append(it.error.responseCode)
                                append(NEW_LINE)
                                append("Server message:")
                                append(NEW_LINE)
                                append(it.error.serverMessage)
                                append(NEW_LINE)
                                append(NEW_LINE)
                            }
                        }
