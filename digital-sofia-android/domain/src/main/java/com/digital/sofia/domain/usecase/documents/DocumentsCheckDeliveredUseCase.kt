...
class DocumentsCheckDeliveredUseCase(
    private val documentsNetworkRepository: DocumentsNetworkRepository,
) {

    companion object {
        private const val TAG = "DocumentsCheckDeliveredUseCaseTag"
    ...
    fun invoke(
        ...
        logDebug("checkDeliveredDocumentStatus evrotrustThreadId: $evrotrustThreadId", TAG)
        checkStatus(threadId = evrotrustThreadId, flow = this@flow)
    ...
    private suspend fun checkStatus(
        ...
        channelFlow {
            while (!isClosedForSend) {
                ...
                send(documentsNetworkRepository.checkDeliveredDocumentStatus(evrotrustThreadId = threadId))
            }
        }.collect {
            it.collect { result ->
                result.onSuccess { model ->
                    logDebug("checkStatus onSuccess status: $model", TAG)
                    when(model) {
                    ...
                    checkStatus(threadId = threadId, flow = flow)
                }.onFailure { failure ->
