 ...
 */
package com.digital.sofia.domain.models.base
...
data class ResultEmittedData<out T>(
    ...
    companion object {
        ...
        fun <T> retry(data: T? = null): ResultEmittedData<T> =
            ResultEmittedData(Status.RETRY, data, null)
...
inline fun <T: Any> ResultEmittedData<T>.onRetry(action: () -> Unit): ResultEmittedData<T> {
    if (status == ResultEmittedData.Status.RETRY) action()
    return this
}