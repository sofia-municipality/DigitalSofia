...
class AuthEnterCodeFlowViewModel(
    ...
    fun getStartDestination(): StartDestination {
        return if (biometricManager.readyToBiometricAuth()) {
            logDebug("to authEnterBiometricFragment", TAG)
            StartDestination(R.id.authEnterBiometricFragment)
        } else {
            logDebug("to authEnterPinFragment", TAG)
            StartDestination(R.id.authEnterPinFragment)
        }
    }
