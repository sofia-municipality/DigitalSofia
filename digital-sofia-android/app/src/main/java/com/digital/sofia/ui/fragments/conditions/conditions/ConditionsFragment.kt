 ...
 **/
package com.digital.sofia.ui.fragments.conditions.conditions
...
class ConditionsFragment : BaseWebViewFragment<ConditionsViewModel>() {
    ...
    override val toolbarNavigationIconRes: Int = R.drawable.ic_back_big

    override val toolbarNavigationTextRes: Int = R.string.back

    override val textAppearance: Int = R.style.TextStyle_24spRegularMainColor
    ...
    override fun onCreated() {
        try {
            ...
            val url = if (!token.isNullOrEmpty() && !refreshToken.isNullOrEmpty()) {
                URL_BASE_WEB_VIEW + "terms-and-conditions/?hideNav=true&token=$token&refreshToken=$refreshToken&lang=$language"
            } else {
                URL_BASE_WEB_VIEW + "terms-and-conditions/?hideNav=true&lang=$language"
            }
