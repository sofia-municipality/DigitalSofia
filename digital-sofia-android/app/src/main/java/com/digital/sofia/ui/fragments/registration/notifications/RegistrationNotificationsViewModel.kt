...
class RegistrationNotificationsViewModel(
    ...
    companion object {
        private const val TAG = "RegistrationNotificationsViewModelTag"
    }

    override val isAuthorizationActive: Boolean = false

    fun proceedNext() {
        preferences.saveUserAskedPermissionBefore()
        findFlowNavController().navigateInMainThread(
            RegistrationNotificationsFragmentDirections.toEnterEgnFragment(),
            viewModelScope
        )
    }
}