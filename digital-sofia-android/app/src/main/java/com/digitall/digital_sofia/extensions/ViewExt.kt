...
fun TextView.setCompoundDrawableTint(
    ...
    compoundDrawables[2]?.let {
        if (colorRight != null) {
            it.colorFilter = PorterDuffColorFilter(context.color(colorRight), PorterDuff.Mode.SRC_IN)
        }
    }
    compoundDrawables[3]?.let {
        if (colorBottom != null) {
            it.colorFilter = PorterDuffColorFilter(context.color(colorBottom), PorterDuff.Mode.SRC_IN)
        }
    }
}
 ...
 */
fun View.setBackgroundColorResource(resource: Int) {
