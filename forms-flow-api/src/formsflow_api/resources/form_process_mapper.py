...
submission_list_model = API.model(
    "SubmissionList",
    {
        "submissions": fields.List(
            fields.Nested(
                API.model(
                    "Submission",
                    {
...
class FormResourceList(Resource):
    ...
    @API.doc(
        params={
            ...
            "formPath": {
                "in": "query",
                "description": "Retrieve form list based on machine name.",
                "default": "",
            }
        }
    )
    ...
    def get():  # pylint: disable=too-many-locals
        ...
        try:
            ...
            form_path: str = dict_data.get("form_path")
            ...
            current_app.logger.debug(f"Schema form_process_mapper_schema - {form_process_mapper_schema}")
            return (
                (
                    {
...
class FormResourceById(Resource):
    ...
    def delete(mapper_id: int):
        ...
        try:
            form_process_mapper = FormProcessMapperService.mark_inactive_and_delete(
            ...
            response, status = (
                formio_service.delete_form(form_process_mapper.form_id, form_io_token),
                HTTPStatus.OK,
            )
...
@cors_preflight("GET, OPTIONS")
@API.route("/<form_path>/submissions", methods=["GET", "OPTIONS"])
class FormioSubmissionsResource(Resource):

    @staticmethod
    @auth.require
    @API.response(200, "OK:- Successful request.", model=submission_list_model)
    @API.response(
        400,
        "BAD_REQUEST:- Invalid request.",
    )
    ...
    def get(form_path):
        try:
            ...
            response = formio_service.get_all_submissions(
                form_path, formio_token, query_params
            )
...
@cors_preflight("GET, OPTIONS")
@API.route("/<form_path>/submission", methods=["GET", "OPTIONS"])
class FormioFindSubmissionResource(Resource):
    ...
    def get(form_path):
        try:
            ...
            formio_token = formio_service.get_formio_access_token()
            query_params = request.query_string.decode()
            response = formio_service.find_and_fetch_submission(
                form_path, formio_token, query_params
            )
