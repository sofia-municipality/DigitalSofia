...
from formsflow_api.services.external import BPMService, EurotrustIntegrationsService
    ...
    def delete(application_id: int, **kwargs):
        try:
            ...
            if application.process_instance_id:
                ...
                process_instance = BPMService.get_process_instance(process_instance_id=application.process_instance_id, token=None)
                if process_instance and process_instance.get("id"):
                    ### 4.2. Get process values
                    process_variables = BPMService.get_process_variables(process_instance_id=application.process_instance_id, token=None)
            ...
            current_app.logger.debug(f"5. From which applications should we delete " + ' '.join(map(str, application_ids_to_delete)))
            ...
            if file_formio_path:
                ...
                for file_submission in related_file_submissions:
                    ...
                    document_transaction = client.get_document_by_submission_id(formio_id=file_id)
...
class ApplicationResourceAvailableForUser(Resource):
    ...
    def get(cls, **kwargs):
        try:
            ...
            bpm_variables = BPMService.get_task_variables(task_id=task_id, token=None)
...
class ApplicationWithdraw(Resource):
    ...
    def post(application_id: int, role:str, **kwargs):
        ...
        ### 3. Application doesn't exist return not found
        ...
        current_app.logger.debug("4. Application exist get process instance id")
        process_variables = BPMService.get_process_variables(application.process_instance_id, token=None)

        ### 5. Generate correct message
        current_app.logger.debug("5. Generate correct message")
        message_name = None
        if role == "owner":
        ...
        current_app.logger.debug("6. Send delete message message")
        ...
        current_app.logger.debug("7. Check is response successful")
        if response != True:
            return {
                "type": "Bad Request Error",
        ...
        ### 8. Try and get withdrawn application id
        current_app.logger.debug("8. Try and get withdrawn application id")
        if role == "owner":
            withdrawn_application_id = process_variables.get("ownerFormApplicationId", {}).get("value")
        elif role == "trustee":
            withdrawn_application_id = process_variables.get("trusteeFormApplicationId", {}).get("value")


        if not withdrawn_application_id:
            ### 9. No additional application to be withdrawn
            current_app.logger.debug("9. No additional application to be withdrawn")
            return "Withdrawn successfully", HTTPStatus.OK

        ### Check if a file exists for the application
        ### 9. Init FormioClient
        current_app.logger.debug("9. Init formio client")
        formio_client = FormioServiceExtended()
        
        ### 10. Generate formio resource path
        formio_file_resource_path = user.tenant_key + "-" + current_app.config.get('FORMIO_FILE_RESOURCE_PATH')
        current_app.logger.debug(f"10. Generate file_path - {formio_file_resource_path}")

        ### 11. Generate token
        current_app.logger.debug(f"11. Generate access token")
        formio_token = formio_client.get_formio_access_token()

        ### 12. Try and get formio resource
        current_app.logger.debug(f"12. Query formio resource by application")
        formio_resource = formio_client.find_and_fetch_submission(
            form_path=formio_file_resource_path,
            formio_token=formio_token,
            data={
                "data.applicationId": withdrawn_application_id
            }
        )

        ### 13. No additional application to be withdrawn
        current_app.logger.debug(f"13. Is there a formio resource to handle")
        if not formio_resource:
            current_app.logger.debug(f"14. No formio resource, withdrawn successfully")
            return "Withdrawn successfully", HTTPStatus.OK
        
        
        ### 14. Getting relevant formio_data
        current_app.logger.debug(f"14. Getting relevant formio_data")
        ...
        ### 15. Withdraw evrotrust document if id is available and status is pending
        current_app.logger.debug(f"15. Withdraw evrotrust document if id is available and status is pending")
        if evrotrust_thread_id and evrotrust_status == 'pending':
        ...
        ### 17. Delete application, application history and draft row within python DB
