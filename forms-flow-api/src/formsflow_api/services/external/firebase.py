...
class FirebaseService:
    ...
    def send_status_change_message(
        ...
        if not status:
            current_app.logger.debug(
                f"Couldn't find status with id - {transaction.status_id}"
            )
            return None
        ...
        if not body:
            current_app.logger.debug(
                f"Couldn't find body matching status - {status.formio_status}"
            )
            return None
        ...
        android_notification = messaging.AndroidNotification(
            title=title, body=localized_body
        )

        android_config = messaging.AndroidConfig(
            data=data_object, notification=android_notification
        )

        message = messaging.Message(
            notification=messaging.Notification(title=title, body=localized_body),
            data=data_object,
        ...
        try:
            ...
            current_app.logger.error(
                f"A value error has occurred when sending message:{err}"
            )
            return None

        return response

    def sent_delivery_message(
        self,
        transaction: DocumentTransaction,
        firebase_user_registration_token: str,
        locale: str = "bg",
    ):
        current_app.logger.info("FirebaseService@sent_delivery_message")
        messages = {
            "en": "You have a new electronic document to receive",
            "bg": "Имате нов електронен документ за получаване",
        }

        body = messages.get(locale)
        ...
        title = "Digitall Sofia"
        android_notification = messaging.AndroidNotification(title=title, body=body)
    ...
    def single_message(self, title: str, body: str, fcm_token: str, data: dict = {}):

        android_notification = messaging.AndroidNotification(title=title, body=body)

        android_config = messaging.AndroidConfig(
            data=data, notification=android_notification
        )
    ...
    def send_multicast_message(
        self, title: str, body: str, registration_tokens: list, data_object: dict = None
    ):
    ...
    def send_user_status_change_message(self, user_data, fcm):
        title = "Digitall Sofia"
        body = "User status change"
        android_notification = messaging.AndroidNotification(title=title, body=body)

        android_config = messaging.AndroidConfig(
            data=user_data, notification=android_notification
        )
        message = messaging.Message(
            notification=messaging.Notification(title=title, body=body),
            data=user_data,
