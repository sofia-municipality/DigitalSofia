...
class DocumentMetaData:
    ...
    def __iterate_recurs(
        ...
        if bool(list(element)):
            ...
            if isinstance(replacement_value, Element):
                element.text = ""
                element.extend(list(replacement_value))
            else:
    ...
    def __get_replacement_from_data(self, matches, data: dict):
        ...
        # print(latest_data)
        if type(latest_data) is dict and last_key == '*':
            elem = Element(keys[-2])
            self.__dict_to_xml_recursive(latest_data, elem)
            return elem
        return default_value if invalid_path else latest_data.get(last_key, default_value)

    def __dict_to_xml_recursive(self, data, elem):
        for key, value in data.items():
           chield = Element(key)
           if type(value) is dict and value:
                self.__dict_to_xml_recursive(value, chield)
           else:
                chield.text = str(value)
