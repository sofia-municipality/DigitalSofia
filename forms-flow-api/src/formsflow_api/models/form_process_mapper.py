...
class FormProcessMapper(AuditDateTimeMixin, AuditUserMixin, BaseModel, db.Model):
    ...
    form_path = db.Column(db.String(100), nullable=True)
    ...
    def create_from_dict(cls, mapper_info: dict) -> FormProcessMapper:
        ...
        try:
            if mapper_info:
                ...
                mapper.form_path = mapper_info["form_path"]
    ...
    def find_all_forms(
        ...
        **filters,
        ...
        if form_path:
            query = query.filter(and_(FormProcessMapper.form_path.ilike('%{}%'.format(form_path))))
