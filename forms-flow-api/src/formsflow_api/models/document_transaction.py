...
class DocumentTransaction(AuditDateTimeMixin, BaseModel, db.Model):
    ...
    status_id = db.Column(
        db.Integer, db.ForeignKey("document_status.id"), nullable=True
    )
    ...
    def notify_user_for_document_delivering(self):
        """
        Get user fcm and send delivery message via FirebaseService
        """
        from formsflow_api.services import KeycloakAdminAPIService, FirebaseService

        current_app.logger.info("DocumentTransaction@notify_user_test")
        current_app.logger.info("Transaction signature source")
        current_app.logger.info(self.signature_source)
        if not self.signature_source and self.signature_source != "digitalSofia":
            return
        ...
        if keycloak_users:
            ...
            if firebase_user_registration_token:
                ...
                firebase_user_registration_token = firebase_user_registration_token[0]
                firebase_client = FirebaseService()
                firebase_client.sent_delivery_message(
                    self, firebase_user_registration_token
                )
